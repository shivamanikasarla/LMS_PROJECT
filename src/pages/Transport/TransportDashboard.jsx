...
const TransportDashboard = () => {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            // Randomly fluctuate student count and route occupancy to simulate assignments/attendance
            setStats(prev => ({
                ...prev,
    ...
    const getPieGradient = () => {
        ...
        // const inactivePct = (vehicleStatus.inactive / total) * 100;
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div className="glass-card" style={{ padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div style={{
                        ...
                        background: stats.systemStatus === 'Operational' ? '#ecfdf5' : '#fff7ed',
                    ...
                    <div>
                        <h3 style={{ margin: 0, fontSize: '16px' }}>System Status: <span style={{ color: stats.systemStatus === 'Operational' ? '#10b981' : '#f59e0b' }}>{stats.systemStatus}</span></h3>
                        <p style={{ margin: 0, fontSize: '13px', color: '#64748b' }}>Live updates enabled • Auto-refreshing every 5s</p>
                    </div>
                </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '24px' }}>
                ...
                <div className="glass-card" style={{ padding: '24px', minHeight: '350px', display: 'flex', flexDirection: 'column' }}>
                    <h3 style={{ margin: '0 0 24px 0', fontSize: '18px' }}>Route Occupancy</h3>
                        ...
                        {routeOccupancy.map((route, i) => {
                            ...
                            return (
                                <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, height: '100%', justifyContent: 'flex-end' }}>
                                    <div style={{ marginBottom: '8px', fontSize: '12px', fontWeight: 'bold', color: '#475569' }}>{pct.toFixed(0)}%</div>
                                    <motion.div
                                        initial={{ height: 0 }}
                                        animate={{ height: `${pct}%` }}
                                        style={{
                                    ...
                                    />
                                    <div style={{ marginTop: '8px', fontSize: '12px', color: '#64748b' }}>{route.route}</div>
                                </div>
                            )
                        })}
                    </div>
                ...
                <div className="glass-card" style={{ padding: '24px', minHeight: '350px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    <h3 style={{ alignSelf: 'flex-start', margin: '0 0 24px 0', fontSize: '18px' }}>Vehicle Status</h3>
                    <div style={{ position: 'relative', width: '200px', height: '200px', borderRadius: '50%', background: getPieGradient() }}>
                        <div style={{
                            position: 'absolute', inset: '30px', background: 'white', borderRadius: '50%',
                            display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column',
                            boxShadow: 'inset 0 2px 5px rgba(0,0,0,0.05)'
                        }}>
                            <span style={{ fontSize: '32px', fontWeight: 'bold', color: '#1e293b' }}>{stats.totalVehicles}</span>
                            <span style={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase' }}>Total Fleet</span>
                        </div>
                    </div>

                    <div style={{ display: 'flex', gap: '24px', marginTop: '32px' }}>
                        <StatusLegend color="#10b981" label="Active" value={vehicleStatus.active} />
                        <StatusLegend color="#f59e0b" label="Maintenance" value={vehicleStatus.maintenance} />
                        <StatusLegend color="#ef4444" label="Inactive" value={vehicleStatus.inactive} />
                    </div>
                </div>
            </div>
        </div>
...
const KpiCard = ({ title, value, icon, color, trend }) => (
    ...
    >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            {/* <span style={{ background: '#f1f5f9', padding: '4px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', color: '#475569' }}>
                This Month
            </span> */}
        </div>
        <div>
