...
const TransportReports = () => {
    ...
    const generateDriverReport = () => {
        return drivers.map(driver => {
            const licenseExpiry = new Date(driver.expiry);
            ...
            return {
                ...
                assignedRoute: driver.assignedRoute || 'Unassigned',
                shift: driver.shift,
                policeVerification: driver.verification?.police ? 'Verified' : 'Pending',
                medicalCheck: driver.verification?.medical ? 'Cleared' : 'Pending'
            };
        });
    };
    ...
    const generateFuelConsumptionReport = () => {
        return fuel.map(log => {
            const vehicle = vehicles.find(v => v.number === log.vehicleId);
